...
class BPlusTree[K: Ordering, V](val order: Int) {
  private var root: Node[K, V] = LeafNode(Vector.empty, Vector.empty, None)
  ...
  def insert(key: K, value: V): Unit = {
      ...
      case LeafNode(keys, values, next) =>
        val (idx, newKeys, newValues) = 
          keys.indexWhere(implicitly[Ordering[K]].gt(_, key)) match {
            case -1 => 
              (keys.size, keys :+ key, values :+ value)// Insert at the end
            case i => 
    ...
    root = insertNode(root) match {
      case (newRoot, Some((splitKey, splitNode))) =>
        InternalNode(Vector(splitKey), Vector(newRoot, splitNode))
      case (newRoot, None) => newRoot
    }
  }
  ...
  def delete(key: K): Unit = {
    ...
    root = deleteNode(root) match {
      case (newRoot, true) if newRoot.isInstanceOf[InternalNode[K, V]] && newRoot.asInstanceOf[InternalNode[K, V]].children.size == 1 =>
        newRoot.asInstanceOf[InternalNode[K, V]].children.head
      case (newRoot, _) => newRoot
    }
  }
...
object BPlusTreeDemo extends App {
  val bpt = new BPlusTree[Int, String](2)
  bpt.insert(10, "Ten")
